...
PORE_DENSITY = 0.1  # Lowered percentage of the polygon area covered by pores
NUM_CLUSTERS = 2  # Focused clustering along the left boundary
...
# Function to generate realistic, irregular pores along the left boundary
def generate_pores_on_left_boundary(polygon, num_pores, img_shape):
    pores = []
    ...
    # Define a left-side boundary constraint
    left_x_max = x_min + (x_max - x_min) // 4  # Limit pores to the leftmost 25% of the polygon width
    
    max_attempts = 50  # Attempts per pore to prevent overlap
    cluster_centers = []
    
    # Generate 2 small clusters only on the left side
    for _ in range(NUM_CLUSTERS):
        cluster_x = random.randint(x_min, left_x_max)
    ...
    for _ in range(num_pores):
        ...
        while attempts < max_attempts:
            # Select a cluster center randomly from left-boundary clusters
            cluster_x, cluster_y = random.choice(cluster_centers)
            rand_x = int(cluster_x + random.randint(-10, 10))  # Keep pores near the left boundary
            rand_y = int(cluster_y + random.randint(-10, 10))
            
            # Ensure pores do not go beyond the left boundary
            rand_x = max(x_min, min(rand_x, left_x_max))
    ...
    for annotation_file in os.listdir(annotation_dir):
        ...
        with open(annotation_path, 'r') as f:
            for line in f:
                ...
                # Generate realistic, irregular pores on the left boundary
                pores = generate_pores_on_left_boundary(points, num_pores, image.shape)
...
output_dir = "F:/Pomodoro/Work/TIME/Script/Thesis-Abbas-Segmentation/PolygontoYOLO/ErrorPlayground/pore4"
