...
BOUNDARY_MARGIN = 3
PORE_RING_THRESHOLD = 4
SMALL_RING_PADDING = 2
LARGE_RING_PADDING = 4
...
def generate_balanced_pores_with_labels(polygon, img_shape):
    ...
    size_ranges = [(1, 2)] * small + [(2, 3)] * medium + [(4, 5)] * large
    ...
    while size_ranges and attempts < 1000:
        ...
        if random.random() < 0.6 and cluster_centers:
            ...
            x = int(cx + random.uniform(5, 20) * np.cos(np.deg2rad(angle_deg)))
            y = int(cy + random.uniform(5, 20) * np.sin(np.deg2rad(angle_deg)))
        else:
